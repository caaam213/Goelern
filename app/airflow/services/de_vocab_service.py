...
from webapp.constants.constants import (
    ALL,
    ALPHABETICAL_ORDER_STR,
    ASC_STR,
    FREQUENCY_STR,
)


class DeVocabService:
    # TODO : Modify it to be modulable and accessible for every languages
    def __init__(self):
        self.df_spark = None

    def get_all_vocabs(self, table_name: str):
        ...
        try:
            ...
            spark = SparkSession.builder.appName(
                "Pandas to PySpark to make queries"
            ).getOrCreate()
    ...
    def order_by_field(self, df: pd.DataFrame, column_name: str, order_type: str):
        return df.orderBy(column_name, ascending=order_type)

    def filter_by_category(self, df: pd.DataFrame, category: str):
        return df.filter(self.df_spark["Category"] == category)

    def get_category_list(self):
        category_list = self.df_spark.select("Category").distinct().collect()
        category_list = [
            category.Category.replace("_", " ") for category in category_list
        ]
